function [fea] = mrmr_mid_d_fix_efi_logDiv(d, f, K, T)
% function [fea] = mrmr_mid_d(d, f, K, T)
% 
% MID scheme according to MRMR
%
% Base by Hanchuan Peng
% April 16, 2003
%
...
for k=2:K,
   ...
   for i=1:ncand,
      t_mi(i) = mi_fix_efi_logDiv(d(:,idxleft(i)), f, T); 
...
c = zeros(1,size(da,2));
for i=1:size(da,2), 
   c(i) = mi_fix_efi_logDiv(da(:,i), dt, T);
end;