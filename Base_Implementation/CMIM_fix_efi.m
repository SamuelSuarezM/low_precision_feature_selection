function selectedFeatures = CMIM_fix_efi(X,Y, topK, T)
% Summary
...
for index_feature = 1:numFeatures
    score_per_feature_uni(index_feature) = mi_fix_efi(X(:,index_feature),Y,T);
end
...
while count<=topK
    ...
    for index_feature_ns = 1:length(not_selected_features)        
        score_per_feature(not_selected_features(index_feature_ns)) = min(score_per_feature(not_selected_features(index_feature_ns)),cmi_fix_efi(X(:,not_selected_features(index_feature_ns)), Y, X(:, selectedFeatures(count-1)),T));        
    end       
