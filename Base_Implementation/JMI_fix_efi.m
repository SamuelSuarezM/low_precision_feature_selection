function [selectedFeatures] = JMI_fix_efi(X_data,Y_labels, topK, T)
% Summary 
...
while count<=topK
    ...
    for index_feature_ns = 1:length(not_selected_features)
            ...
            score_per_feature(not_selected_features(index_feature_ns)) = p_binf2dec(p_add_binf(fix_dec2binfm(score_per_feature(not_selected_features(index_feature_ns)),T),fix_dec2binfm(mi_fix_efi([X_data(:,not_selected_features(index_feature_ns)),X_data(:, selectedFeatures(count-1))], Y_labels, T),T)));
